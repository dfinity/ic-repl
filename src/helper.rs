...
use crate::utils::{fetch_state_tree_path_, random_value, str_to_principal};
use candid::{
...
async fn fetch_actor(agent: &Agent, canister_id: Principal) -> anyhow::Result<CanisterInfo> {
    let response =
        fetch_state_tree_path_(agent, "canister", canister_id, "metadata/candid:service").await;
    let profiling = fetch_state_tree_path_(agent, "canister", canister_id, "metadata/name")
        .await
        .ok()
        .as_ref()
